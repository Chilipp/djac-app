...
describe('Application life cycle test', function () {
    ...
    async function login(username, password) {
        ...
        await browser.findElements(By.xpath('//a[@class="dropdown-item" and contains(., "Log out")]')).then(function(elems) {
            browser.get(`https://${app.fqdn}/accounts/logout/`);
            browser.sleep(2000);
            browser.get(`https://${app.fqdn}/accounts/login/`);
            browser.sleep(2000);
        });

        await browser.wait(until.elementLocated(By.xpath('//input[@name="login"]')));
        ...
        await browser.findElements(By.xpath('//input[@value="Create Space"]')).then(function(elems) {
            browser.findElement(By.xpath('//input[@value="Create Space"]')).click();
            browser.sleep(3000);
        });
...
//        await browser.wait(until.elementLocated(By.xpath('//p[@class="card-text" and contains(., "Create a new recipe directly in Tandoor.")]')), TIMEOUT);
    ...
    async function getMainPage() {
        ...
        await browser.sleep(5000);
        await browser.wait(until.elementLocated(By.id('collapse_advanced_search')), TIMEOUT);
    }
